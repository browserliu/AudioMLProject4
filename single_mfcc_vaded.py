...
mfcc_dir_path = '/mnt/alderaan/mlteam3/Assignment4/data/vox_test/'
output_mfcc_path = '/mnt/alderaan/mlteam3/Assignment4/data/concat_vox_test'
    ...
    for root, dirs, files in os.walk(mfcc_dir_path):
        ...
        for file in files:
            if (file.lower().endswith('.dat')):
                ...
                mfcc_file_path = os.path.join(mfcc_dir_path,file)
                mfcc_vector =  unpackMfccVector(mfcc_file_path)
                speech_vector_final = np.vstack((speech_vector_final,mfcc_vector))

                mfcc_vector_output_file = os.path.join(output_mfcc_path, 'voxforge_de_test_39D_mfcc.dat')
