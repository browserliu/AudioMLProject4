...
for probe in speaker_list:
	...
	for probe_ivector in probe_ivectors:
		...
		for model in speaker_list:
			...
			#wccn
			model_ivectors_wccn_projected = map(wccn_machine.forward, model_ivectors)
			probe_ivector_wccn_projected = wccn_machine.forward(probe_ivector)

			score = cosine_score(model_ivectors_wccn_projected, probe_ivector_wccn_projected)
...
df.to_csv("results/ivector_wccn_ENubm_balanced30fold.csv")